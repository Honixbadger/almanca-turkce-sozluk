#!/usr/bin/env python3
"""Clean `esanlamlilar` entries with missing targets, self-loops and non-reciprocal links."""
...
BACKLINK_FIELDS = ("esanlamlilar", "sinonim")
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--report-path",
        type=Path,
        default=DEFAULT_REPORT_DIR / "esanlamlilar_links.json",
    )
    ...
    parser.add_argument("--keep-max", type=int, default=20)
...
def main() -> int:
    ...
    for rec_idx, record in enumerate(records):
        ...
        values = to_list(record.get("esanlamlilar"))
        ...
        if args.apply_fixes:
            if cleaned:
                record["esanlamlilar"] = cleaned
            else:
                record.pop("esanlamlilar", None)
